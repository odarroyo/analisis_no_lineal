...
node_info_updated = np.column_stack((nodes_tags_updated,coordx_updated,coordy_updated))

# create an interactive table that allows the user input masses and nodal loads at each node in the node_info_updated table.
# user can input masses and nodal loads at each node. 
# Mass in ton mass and nodal loads in kN
# for masses, a single value that will be applied to the node as: 
# mass(nodes_tags_updated[i],mass_input_by_user_at_node,mass_input_by_user_at_node,0)

# for nodal loads, three values that will be applied to the node as:
# load(nodes_tags_updated[i],Fx_input_by_user_at_node,Fy_input_by_user_at_node,Mz_input_by_user_at_node)

#%% NEW SECTION FOR THE PUSHOVER NORMALIZATION}
eigen(len(coordy)-1)
modalProperties('-print','-unorm','-return')
